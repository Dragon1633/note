...
> ```c
> # 克隆远程仓库的 develop 分支
> git clone -b develop https://github.com/example/repo.git --depth 1		//--depth 1：只克隆最近一次提交（浅克隆）
> ```
...
> 您可以`tasks.json`使用类似参数而`"${workspaceFolder}/*.cpp"`来修改您的文件以构建多个 C++ 文件`${file}`。这将构建`.cpp`当前文件夹中的所有文件。
>
> `"${fileDirname}\\${fileBasenameNoExtension}.exe"`您还可以通过替换为硬编码文件名来修改输出文件`"${workspaceFolder}\\myProgram.exe"`。

**lunch.json**文件

用于配置 **调试会话**，定义如何启动和调试程序。例如：
...
配置后，可以直接点击 VS Code 左侧的“运行和调试”图标来启动调试。
...
[控制台](https://dashboard.ngrok.com/get-started/setup/windows)
...
访问 [http://localhost:4040](http://localhost:4040/) 可实时监控谁调用了你的API



> 1. **每次重启Ngrok都会变网址**
>
>    - 临时用途：直接复制新网址到Coze工作流
>    - 长期用途：付费升级Ngrok计划可固定域名
>
> 2. **基础身份验证（防滥用）**
>    在Ngrok命令中添加用户名密码：
>
...
>
>    ```
>    headers = {"Authorization": "Basic  base64编码的用户名密码"}
>    ```
